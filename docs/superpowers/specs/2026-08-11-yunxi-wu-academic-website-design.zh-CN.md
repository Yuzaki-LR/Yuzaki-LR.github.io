...
- 当前阶段：本科生
- 公开电子邮箱：**yxw1331@student.bham.ac.uk**
- 网站语言：英文
...
除非用户另行提供或确认，否则不得加入毕业年份、成绩、奖学金、奖项、其他机构身份、所在城市、其他联系方式、社交账号或其他个人事实。
...
- 宽屏时，个人资料和链接位于左侧栏；
- 主要学术内容位于右侧；
- 标题和大部分正文以 Times New Roman 为主要字体；
- 使用细分隔线和低饱和度蓝灰色强调色；
...
- 分隔线和边框：`#dfe2e6`
- 主要强调色：`#2d587a`
- 主要标题和正文：`Times New Roman`、Times、serif 回退字体
- 小型导航、元信息和界面标签可以使用 Arial 或 `Helvetica Neue` 作为次要无衬线字体

绝大部分可见文字使用 Times New Roman。使用系统字体，不依赖远程字体服务。悬停效果仅限链接颜色或下划线变化；键盘焦点必须清晰可见。
...
以下资料尚未提供。网站可以先在本地完成，但不得用虚假占位链接代替缺失信息。已经确认的公开联系邮箱为 `yxw1331@student.bham.ac.uk`，网站中应将其显示为可正常使用的 `mailto:` 链接。
...
第一版不设置 CV 页面，也不提供 CV 下载。此外，第一版不需要博客、CMS、评论、访问追踪、主题切换、中英双语、联系表单、搜索或自动生成的社交分享图。
...
- About、Projects 和 Research 路由均可成功构建；
- 三个项目详情页均使用已经确认的证据模板；
