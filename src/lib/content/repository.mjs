...
async function defaultContentRoot(io) {
  if (process.env.NODE_ENV === 'test' && process.env.TEST_SITE_CONTENT_ROOT) return path.resolve(process.env.TEST_SITE_CONTENT_ROOT);
  try { if ((await io.stat(moduleContentRoot)).isDirectory()) return moduleContentRoot; } catch { /* Astro prerender bundles this module outside src. */ }
  return path.resolve(process.cwd(), 'src/content');
}

function fail(message) { throw new Error(message); }
async function realDirectory(value, message, io) {
...
async function regularFile(root, relative, message, io) {
  const target = path.resolve(root, ...relative.split('/'));
  if (!contained(root, target)) fail(message);
  let resolved;
  try { resolved = await io.realpath(target); } catch { fail(message); }
...
async function loadProjectImages(root, slug, document, io) {
  const images = [];
  for (const source of imageSources(document)) {
    ...
    const sourcePath = await regularFile(root, `projects/${slug}/images/${name}`, 'project image escapes content root', io);
...
async function loadProjectRecords(root, io) {
  const directory = path.join(root, 'projects');
  try { await io.lstat(directory); } catch { return []; }
  const records = [];
  for (const entry of await io.readdir(directory, { withFileTypes: true })) {
    ...
    const sourcePath = await regularFile(root, `projects/${entry.name}/index.md`, 'project record is invalid', io);
    const document = parseProjectFile(await io.readFile(sourcePath, 'utf8'));
    document.slug = entry.name;
    validateProject(document);
    records.push({ slug: entry.name, document, sourcePath, images: await loadProjectImages(root, entry.name, document, io) });
  }
  return records.sort((a, b) => a.document.frontmatter.order - b.document.frontmatter.order);
}
export async function loadProjects({ contentRoot, io = nodeFs } = {}) {
  contentRoot ??= await defaultContentRoot(io);
  const root = await realDirectory(contentRoot instanceof URL ? contentRoot : contentRoot, 'content root is invalid', io);
  return (await loadProjectRecords(root, io)).map(({ document }) => document);
}
async function loadResearch(root, io) {
  ...
  try {
    for (const entry of await io.readdir(directory, { withFileTypes: true })) {
      ...
      const sourcePath = await regularFile(root, `research/${entry.name}`, 'research record is invalid', io);
      const document = parseResearchFile(await io.readFile(sourcePath, 'utf8'));
...
export async function computeRepositoryManifest(repositoryRoot, { io = nodeFs } = {}) {
  const root = await realDirectory(repositoryRoot, 'repository root is invalid', io);
  const files = [];
  async function visit(directory) {
    for (const entry of await io.readdir(directory, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) await visit(target);
        ...
        const resolved = await io.realpath(target);
  ...
  for (const file of files) { hash.update(file); hash.update('\0'); hash.update(await io.readFile(path.join(root, ...file.split('/')))); hash.update('\0'); }
  return { hash: hash.digest('hex'), files };
}
export async function loadSiteRepository({ contentRoot, io = nodeFs } = {}) {
  contentRoot ??= await defaultContentRoot(io);
  const root = await realDirectory(contentRoot instanceof URL ? contentRoot : contentRoot, 'content root is invalid', io);
  const sitePath = await regularFile(root, 'site.yml', 'site configuration is invalid', io);
  const aboutPath = await regularFile(root, 'pages/about.md', 'about page is invalid', io);
  const site = parseSiteYaml(await io.readFile(sitePath, 'utf8'));
  validateSite(site);
  const about = parsePageFile(await io.readFile(aboutPath, 'utf8'));
  validatePage(about);
  const [projects, research, manifest] = await Promise.all([loadProjectRecords(root, io), loadResearch(root, io), computeRepositoryManifest(root, { io })]);
  ...
  if (site.avatar.mode === 'image') {
    ...
    const sourcePath = await regularFile(root, `site-images/${name}`, 'site avatar escapes content root', io);
    const bytes = await io.readFile(sourcePath);
